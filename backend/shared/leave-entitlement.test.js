...
test("calculates group allowance based on age and child status", () => {
    assert.equal(getAnnualLeaveGroupAllowanceHours({ birthDate: "1995-01-01", hasChild: false, year: 2024 }), 20 * HOURS_PER_WORKDAY);
    assert.equal(getAnnualLeaveGroupAllowanceHours({ birthDate: "1985-01-01", hasChild: false, year: 2024 }), 25 * HOURS_PER_WORKDAY);
    assert.equal(getAnnualLeaveGroupAllowanceHours({ birthDate: null, hasChild: true, year: 2024 }), 25 * HOURS_PER_WORKDAY);
});
...
test("caps carry-over by the group allowance limit", () => {
    assert.equal(computeCarryOverHours({
...
//# sourceMappingURL=leave-entitlement.test.js.map