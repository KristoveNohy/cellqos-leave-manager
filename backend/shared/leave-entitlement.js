...
export function getAnnualLeaveGroupAllowanceHours({ birthDate, hasChild, year, }) {
    if (hasChild) {
...
function roundLeaveHours(value) {
    return Math.round(value * 100) / 100;
}
export function computeAnnualLeaveAllowanceHours({ birthDate, hasChild, year, employmentStartDate, manualAllowanceHours, accrualPolicy = "YEAR_START", }) {
...
export function computeCarryOverHours({ previousAllowance, previousUsed, carryOverLimit, }) {
    const remaining = Math.max(0, previousAllowance - previousUsed);
    return Math.min(remaining, carryOverLimit);
}
//# sourceMappingURL=leave-entitlement.js.map